"""REST endpoints for device pairing and management.

Two boundaries:
- Device-facing (no Clerk): ``/api/device/register``, ``/api/device/poll-pairing``
- User-facing (Clerk bearer): ``/api/pairing-info/{code}``, ``/api/device/complete-pairing``,
  ``/api/devices``, ``/api/device/{id}/unlink``, ``/api/device/{id}/rename``
"""
...
from storage.devices import DeviceRegistry, DeviceRegistryError
...
TRUST_UNSIGNED_CLERK_JWT = os.getenv("TRUST_UNSIGNED_CLERK_JWT", "1") == "1"

if DEVICE_JWT_SECRET == "dev-device-jwt-secret-change-me":
    logger.warning("DEVICE_JWT_SECRET is using the local development fallback.")


def get_device_registry() -> DeviceRegistry:
    """Overridden in main.py to inject the configured singleton."""
...
async def current_clerk_user_id(authorization: str | None = Header(default=None)) -> str:
    """Resolve a Clerk user_id from a bearer token. Dev shortcuts via AUTH_DEV_USER_ID
    and the ``dev:<id>`` token prefix are kept for local pairing UI testing.
    """
